...
def find_top_k_chunks(embedding_model, query, chunks, k=5):
    ...
    sorted_indices = sorted(range(len(scores[0])), key=lambda i: scores[0][i], reverse=True)
    return [chunks[i] for i in sorted_indices[:k]]

def search_context(query, chunks, embedding_model, k=5, verbose=False, reverse_chunk_indices=False):
...
if __name__ == "__main__":
    ...
    embedding_model = FlagModel(embedding_model_path, 
